...
# client and server.
app = Flask(__name__)
app.config['SECRET_KEY'] = os.getenv('EDUQUEST_SECRET_KEY') or os.urandom(32)
app.config['YOUTUBE_API_KEY'] = os.getenv('EDUQUEST_YOUTUBE_API_KEY', '')
...
def login_route():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Unable to connect to the learning database. Check the database settings and try again.', 'danger')
            return redirect(url_for('login_route'))
...
def handle_edit_message(data):
    ...
    emit('message_edited', {
        ...
        'username': data.get('username', session.get('username', 'Guest'))
...
def handle_join_screen_share(data):
    """Add a viewer to an existing screen-share signaling room."""
    room = data.get('room')
    if room:
        join_room(room)
