...
document.getElementById('delete-selected-btn')?.addEventListener('click', function () {
    ...
    checkboxes.forEach(checkbox => {
        if (checkbox.checked) {
            anyChecked = true;
            const fileLink = checkbox.closest('li').querySelector('a');
            socket.emit('deleteFile', fileLink?.textContent || '');
...
document.getElementById('open-screenshare-btn')?.addEventListener('click', function () {
...
function generateRoomCode() {
    ...
    const roomCodeElement = document.getElementById('generated-code');
    roomCodeElement.classList.add('visible');
...
document.getElementById('join-room-btn')?.addEventListener('click', function () {
    ...
    if (roomCode) {
        ...
        document.getElementById('generated-code').classList.add('visible');
...
const API_KEY = window.youtubeApiKey || '';

// Define a list of bad words for censorship
const badWords = ["bc", "mc","drugs","smoking"]; // Add your bad words here
let showVideos = false; // Related video display is temporarily disabled.
...
function trackKeywords(message) {
    ...
    if (trending.length) {
        ...
        if (topic !== currentTopic && topic !== "***") {
            ...
            // Related video fetching is temporarily disabled.
        } else {
...
// Related video display is temporarily disabled.
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tour.addStep({
        ...
        document.getElementById('start-tour-btn').addEventListener('click', function() {
            // The tour is opt-in from the sidebar so it does not obstruct the workspace on load.
                });
